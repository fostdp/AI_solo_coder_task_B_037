...
public class SpectrumScanner : ISpectrumScanner
{
        ...
        CancellationToken stoppingToken)
    {
        ...
        var interferences = await DetectAndAddInterferencesAsync(frequencyPoints, powerLevels);
        ...
        var gpuAccelerated = _gpuFftWorker?.IsGpuAvailable ?? false;
    ...
    private async Task<List<InterferenceInfo>> DetectAndAddInterferencesAsync(
        double[] frequencies,
        double[] powerLevels)
    {
        ...
        foreach (var interference in interferences)
        {
            ...
            if (interference.IsWideband && _gpuFftWorker != null && _options.EnableGpuAcceleration)
            {
                await ProcessWidebandInterferenceWithGpuAsync(interference, frequencies, powerLevels);
            }
        }
    ...
    private async Task ProcessWidebandInterferenceWithGpuAsync(
        ...
        double[] powerLevels)
    {
        try
        {
            ...
            var fftSize = (int)Math.Pow(2, Math.Ceiling(Math.Log2(powerLevels.Length)));
            var timeDomainData = new double[fftSize];
            for (int i = 0; i < Math.Min(powerLevels.Length, fftSize); i++)
            {
                timeDomainData[i] = Math.Pow(10, powerLevels[i] / 20.0);
            }

            var batchRequests = new List<FftCalculationRequest>();
            for (int s = 0; s < numSubbands; s++)
            {
                batchRequests.Add(new FftCalculationRequest
                {
                    TimeDomainData = (double[])timeDomainData.Clone(),
                    FftSize = fftSize,
                    ApplyWindow = true,
            ...
            var results = await _gpuFftWorker.BatchCalculateFftAsync(batchRequests);

            if (results.All(r => r.GpuAccelerated))
            {
                _logger.LogDebug(
                    "Wideband interference processed with GPU acceleration: {Count} subbands",
                    numSubbands);
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "GPU acceleration failed for wideband interference, falling back to CPU");
        }
    }

    public List<InterferenceInfo> DetectInterferencesFromSpectrum(
        double[] frequencies,
        double[] powerLevels)
    {
        var interferences = new List<InterferenceInfo>();
    ...
    public double CalculateSpectralSecondMoment(
        double[] frequencies,
        double[] powerLevels,
        int startIdx,
        int endIdx,
    ...
    public double CalculateSpectralFlatness(
        double[] powerLevels,
        int startIdx,
        int endIdx)
    {
        ...
        {
            sum += powerLevels[i];
            sumOfSquares += powerLevels[i] * powerLevels[i];
        }

        var mean = sum / count;
        var variance = (sumOfSquares / count) - (mean * mean);

        return 1.0 / (1.0 + Math.Sqrt(Math.Abs(variance)) / 10.0);
    }

    public double CalculateNoiseFloor(double[] powerLevels)
    {
    ...
    public double CalculateSFDR(double[] powerLevels, double noiseFloor)
    {
    ...
    public async Task<(double[] angles, double[] depths)> CalculateNullSteeringWeightsAsync(
        Guid stationId,
        ...
        CancellationToken stoppingToken)
    {
        ...
        if (interferenceDirectionsDeg.Length > 0)
        {
            ...
            {
                var channel = channelList[c];
                var totalPhase = (double)(channel.CalibrationCoeffPhase ?? 0.0m) + finalPhases[c];

                while (totalPhase > Math.PI) totalPhase -= 2 * Math.PI;
                while (totalPhase < -Math.PI) totalPhase += 2 * Math.PI;

                channel.CalibrationCoeffPhase = (decimal)totalPhase;
    ...
    public double[] CalculateWidebandNullSteering(
        double directionRad,
        InterferenceInfo interference,
    ...
    public double[] CalculateMVDRWeights(double directionRad, List<Channel> channels)
    {
        var numChannels = channels.Count;
    ...
    public void ApplyDiagonalLoading(List<Channel> channels, double[] interferenceDirectionsDeg)
    {
        ...
        {
            var channel = channels[i];
            var currentPhase = (double)(channel.CalibrationCoeffPhase ?? 0.0m);

            var loadedPhase = currentPhase * (1 - loadingFactor) +
                             loadingFactor * currentPhase * 0.9;

            channel.CalibrationCoeffPhase = (decimal)loadedPhase;
        }
    }
        ...
        CancellationToken stoppingToken)
    {
        ...
        return _scanRepo != null
            ? await _scanRepo.AddAsync(record, stoppingToken)
            : record;
    }
    ...
    {
        return _scanRepo != null
            ? _scanRepo.GetByStationIdAndTimeRangeAsync(stationId, startTime, endTime, stoppingToken)
            : Task.FromResult<IReadOnlyList<SpectrumScanRecord>>(new List<SpectrumScanRecord>());
    }
        ...
        CancellationToken stoppingToken)
    {
        ...
        var channels = _channelRepo != null
            ? (await _channelRepo.GetByStationIdAsync(stationId, stoppingToken)).ToList()
            : new List<Channel>();
