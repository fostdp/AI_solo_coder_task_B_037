...
public class CoSiteInterferenceAnalyzer : ICoSiteInterferenceAnalyzer
{
    ...
    private readonly CouplingMatrixWorker? _couplingMatrixWorker;

    public CoSiteInterferenceAnalyzer(
        ILogger<CoSiteInterferenceAnalyzer> logger,
        IOptions<CoSiteInterferenceOptions> options,
        IMediator? mediator = null,
        CouplingMatrixWorker? couplingMatrixWorker = null)
    {
        _logger = logger;
        _options = options.Value;
        _mediator = mediator;
        _couplingMatrixWorker = couplingMatrixWorker;
    }
    ...
    public IReadOnlyList<CoSiteAntenna> OptimizeAntennaList(IReadOnlyList<CoSiteAntenna> antennas)
    {
    ...
    public Dictionary<Guid, int> ClusterAntennasByDistance(IReadOnlyList<CoSiteAntenna> antennas)
    {
    ...
    public static double CalculateAntennaDistance(CoSiteAntenna a, CoSiteAntenna b)
    {
    ...
    public static CoSiteAntenna CreateRepresentativeAntenna(List<CoSiteAntenna> group)
    {
    ...
    public double CalculateIsolation(
    ...
    public double CalculateIsolationApproximate(
        double distanceMeters,
        double azimuthDeg,
        double frequencyMhz,
        double interfererStartMhz,
    ...
    public static double CalculateMutualCouplingLoss(
        double distanceMeters,
        double azimuthDeg,
        double elevationDeg,
        double heightOffsetMeters,
    ...
    private async Task<CoSiteInterferenceResult> AnalyzeSingleInterferenceAsync(
        CoSiteInterferenceRequest request,
        ...
        CancellationToken stoppingToken)
    {
        ...
        if (_couplingMatrixWorker != null)
        {
            var selfAntenna = new CoSiteAntenna
            {
                ...
                OperatorName = "Self",
                AntennaType = "Self",
                FrequencyStartMhz = request.SelfFrequencyStartMhz,
                FrequencyEndMhz = request.SelfFrequencyEndMhz,
                TransmitPowerDbm = 0,
            ...
            var couplingRequest = new CouplingCalculationRequest
            {
                AntennaA = interferingAntenna,
                AntennaB = selfAntenna,
                SelfStartMhz = request.SelfFrequencyStartMhz,
                SelfEndMhz = request.SelfFrequencyEndMhz
            };

            isolationDb = await _couplingMatrixWorker.CalculateIsolationAsync(couplingRequest, stoppingToken);
        }
        else
        {
            isolationDb = CalculateIsolation(
        ...
        return new CoSiteInterferenceResult
        {
    ...
    public static double CalculatePolarizationLoss(double elevationDeg)
    {
    ...
    public static double CalculateFrequencyOverlap(
    ...
    public static double CalculateOffAxisAttenuation(double azimuthDeg, double elevationDeg)
    {
    ...
    public double CalculateCouplingCoefficient(
        double isolationDb,
        double interfererStartMhz,
        double interfererEndMhz,
        double selfStartMhz,
    ...
    public static (double x, double y, double z) CalculateInterferenceVector(
        double distanceMeters,
        double azimuthDeg,
        double elevationDeg)
    {
    ...
    public static (double start, double end) CalculateAffectedBand(
    ...
    public virtual async Task<object> SaveInterferenceRecordAsync(
        Guid stationId,
        CoSiteAntenna interferingAntenna,
        ...
        CancellationToken stoppingToken)
    {
        ...
        return new
        {
            ...
            AffectedBandStartMhz = result.AffectedBandStartMhz,
            AffectedBandEndMhz = result.AffectedBandEndMhz,
            MeasurementTime = DateTime.UtcNow,
            CreatedAt = DateTime.UtcNow
        };
    }

    public virtual Task<IReadOnlyList<object>> GetInterferenceHistoryAsync(
        Guid stationId,
        DateTime startTime,
        DateTime endTime,
        CancellationToken stoppingToken)
    ...
    public virtual Task<IReadOnlyList<CoSiteAntenna>> GetCoSiteAntennasAsync(
        Guid stationId,
        CancellationToken stoppingToken)
    {
        return Task.FromResult<IReadOnlyList<CoSiteAntenna>>(new List<CoSiteAntenna>().AsReadOnly());
    }

    public double[,] BuildCouplingMatrix(IReadOnlyList<CoSiteAntenna> antennas)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
                ...
                else
                {
                    ...
                    matrix[i, j] = CalculateMutualCouplingLoss(
                        distance,
