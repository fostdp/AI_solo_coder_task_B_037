...
public class DeformationMonitor : IDeformationMonitor
{
        ...
        foreach (var sensorData in processedSensorData)
        {
            try
            {
                ...
                if (result.ExceedsThreshold && _options.AutoBeamCorrection)
                {
                    ...
                    result = result with { CorrectionApplied = true };
    ...
    internal IReadOnlyList<SensorData> PreprocessSensorData(IReadOnlyList<SensorData> sensorDataList)
    {
    ...
    internal void DetectAndFlagAnomalies(List<SensorData> sensorDataList)
    {
    ...
    internal void InterpolateMissingData(List<SensorData> sensorDataList)
    {
    ...
    internal static double CalculateStdDev(IEnumerable<double> values)
    {
    ...
    private async Task<DeformationResult> AnalyzeSensorDataAsync(
        Guid stationId,
        ...
        CancellationToken stoppingToken)
    {
        ...
        var displacementMm = await _femWorker.CalculateDisplacementFEMAsync(
            sensorData.SensorIndex,
            tiltMagnitude,
    ...
    internal double CalculateDisplacementFEM(
    ...
    public virtual Task<DeformationRecord> SaveDeformationRecordAsync(
        Guid stationId,
        SensorData sensorData,
    ...
    public virtual Task ApplyBeamCorrectionAsync(
        Guid stationId,
    ...
    public virtual Task<IReadOnlyList<DeformationRecord>> GetDeformationHistoryAsync(
        Guid stationId,
        DateTime startTime,
        DateTime endTime,
        CancellationToken stoppingToken)
    {
        return Task.FromResult<IReadOnlyList<DeformationRecord>>(Array.Empty<DeformationRecord>());
    }
}
...
public record DeformationThresholdExceededEvent(
    ...
    double CalculatedDisplacementMm,
    double CorrectionAngleAzimuth,
    double CorrectionAngleElevation,
    DateTime OccurredAt);
