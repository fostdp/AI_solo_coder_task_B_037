...
vi.mock('@/api', () => ({
  default: {
    getCoSiteAntennas: vi.fn().mockResolvedValue(mockAntennas),
    getCoSiteInterferenceRecords: vi.fn().mockResolvedValue(mockInterferenceRecords),
    getInterference3DVectors: vi.fn().mockResolvedValue([])
  }
}))

describe('InterferenceAnalyzer 核心算法测试', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
