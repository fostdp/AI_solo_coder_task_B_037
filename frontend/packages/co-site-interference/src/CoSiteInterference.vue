...
import { ref, onMounted, computed, watch, withDefaults, defineEmits } from 'vue'
...
import { getStatusColor, rgba } from './types'
import {
...
const loadAntennas = async () => {
  ...
  try {
    antennas.value = await mockApi.getCoSiteAntennas(currentStationId.value)
  } catch (e) {
    antennas.value = generateCoSiteAntennas(currentStationId.value, 3)
  }
}
...
const loadInterferenceRecords = async () => {
  if (!currentStationId.value) return
  try {
    interferenceRecords.value = await mockApi.getInterferenceRecords(currentStationId.value)
  } catch (e) {
    interferenceRecords.value = generateInterferenceRecords(currentStationId.value, 20)
  }
}
...
const loadInterferenceVectors = async () => {
  if (!currentStationId.value) return
  try {
    interferenceVectors.value = await mockApi.getInterference3DVectors(currentStationId.value)
  } catch (e) {
...
const loadChannelStatuses = async () => {
  if (!currentStationId.value) return
  try {
    channelStatuses.value = await mockApi.getChannelStatuses(currentStationId.value)
  } catch (e) {
...
const runAnalysis = async () => {
  ...
  try {
    await mockApi.analyzeInterference(currentStationId.value)
    await Promise.all([loadAntennas(), loadInterferenceRecords(), loadInterferenceVectors()])
    emit('analysis-complete', interferenceRecords.value)
  } finally {
...
const saveAntenna = async () => {
  ...
  try {
    if (formData.value.id) {
      await mockApi.updateCoSiteAntenna(formData.value.id, formData.value)
      emit('antenna-updated', formData.value as CoSiteAntenna)
    } else {
      const newAntenna = await mockApi.createCoSiteAntenna({
        ...formData.value,
        stationId: currentStationId.value
      } as CoSiteAntenna)
      emit('antenna-added', newAntenna)
    }
...
const deleteAntenna = async (id: string) => {
  ...
  try {
    await mockApi.deleteCoSiteAntenna(id)
    emit('antenna-deleted', id)
    await loadAntennas()
  } catch (e) {
    alert('删除失败')
  }
...
const mockApi = {
  getBaseStations: async () => {
    ...
    return generateBaseStations(10)
  },
  getCoSiteAntennas: async (stationId: string) => {
    await new Promise(resolve => setTimeout(resolve, 200))
    return generateCoSiteAntennas(stationId, 3)
  },
  getInterferenceRecords: async (stationId: string) => {
    await new Promise(resolve => setTimeout(resolve, 200))
    return generateInterferenceRecords(stationId, 20)
  },
  getInterference3DVectors: async (stationId: string) => {
    await new Promise(resolve => setTimeout(resolve, 200))
    return generateInterference3DVectors(stationId)
  },
  getChannelStatuses: async (stationId: string) => {
    await new Promise(resolve => setTimeout(resolve, 200))
    return generateChannelStatuses(stationId)
  },
  analyzeInterference: async (stationId: string) => {
    await new Promise(resolve => setTimeout(resolve, 1000))
    return { success: true, stationId }
  },
  createCoSiteAntenna: async (antenna: CoSiteAntenna) => {
    await new Promise(resolve => setTimeout(resolve, 200))
    return { ...antenna, id: `cosite-antenna-${Date.now()}` }
  },
  updateCoSiteAntenna: async (id: string, antenna: Partial<CoSiteAntenna>) => {
    await new Promise(resolve => setTimeout(resolve, 200))
    return { ...antenna, id } as CoSiteAntenna
  },
  deleteCoSiteAntenna: async (id: string) => {
    await new Promise(resolve => setTimeout(resolve, 200))
