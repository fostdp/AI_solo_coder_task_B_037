...
describe('SpectrumScanner 核心算法测试', () => {
  ...
  describe('WebGL支持检测测试', () => {
    it('detectWebGLSupport - 正确检测WebGL支持', () => {
      const mockCanvas = {
      ...
      document.createElement = vi.fn(() => mockCanvas as unknown as HTMLCanvasElement)

      try {
        const result = !!(
          (typeof window !== 'undefined' && (window as any).WebGLRenderingContext) &&
          (mockCanvas.getContext('webgl') || mockCanvas.getContext('experimental-webgl'))
        )
    ...
    it('detectWebGLSupport - 不支持时返回false', () => {
      const mockCanvas = {
