    ...
    datasets: [
      {
        ...
        backgroundColor: sorted.map(r =>
          r.efficiencyPercent >= props.efficiencyThreshold
            ? rgba('#10b981', 0.7)
            : rgba('#ef4444', 0.7)
        ),
      ...
      {
        label: `阈值 (${props.efficiencyThreshold}%)`,
        data: sorted.map(() => props.efficiencyThreshold),
        borderColor: '#dc2626',
        borderDash: [5, 5],
        pointRadius: 0,
        type: 'line' as const,
...
const historyChartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      ...
      {
        label: `阈值 (${props.efficiencyThreshold}%)`,
        data: historyData.value.timePoints.map(() => props.efficiencyThreshold),
        borderColor: '#dc2626',
        borderDash: [5, 5],
        pointRadius: 0,
        fill: false
...
const loadRecords = async () => {
  ...
  try {
    records.value = await mockApi.getPaEfficiencyRecords(currentStationId.value, true)
  } catch (e) {
...
const loadReplacementSummaries = async () => {
  try {
    replacementSummaries.value = await mockApi.getPaReplacementSummaries()
  } catch (e) {
...
const loadChannelPanel = async (channelId: string) => {
  try {
    panelData.value = await mockApi.getPaChannelPanelData(channelId)
  } catch (e) {
    panelData.value = {
      channelId,
      ...
      trend: -0.15,
      efficiencyThreshold: props.efficiencyThreshold,
      efficiencyHistory: []
    }
  }
}
...
const loadHistoryData = async (channelId: string) => {
  try {
    historyData.value = await mockApi.getPaEfficiencyHistory(channelId, historyHours.value)
  } catch (e) {
...
const runEvaluation = async (channelId: string) => {
  ...
  try {
    const result = await mockApi.evaluatePaEfficiency(
    ...
    emit('evaluation-complete', { channelId, efficiency: result.efficiency })
    await Promise.all([
      loadRecords(),
...
const createWorkOrder = (channelId: string) => {
  emit('work-order-create', channelId)
}
...
<template>
  <div class="pa-efficiency-tracker">
    <div class="panel-header">
      ...
      <div class="stats-cards">
        ...
        <div class="stat-card" :class="{ 'warning': minEfficiency < props.efficiencyThreshold }">
    ...
    <div class="tab-switcher" v-if="props.showOverview || props.showChannelDetail || props.showHistory">
      <button v-if="props.showOverview" :class="{ active: activeTab === 'overview' }" @click="activeTab = 'overview'">
        总览
      </button>
      <button v-if="props.showChannelDetail" :class="{ active: activeTab === 'channel' }" @click="activeTab = 'channel'" :disabled="!selectedChannelId">
        通道详情
      </button>
      <button v-if="props.showHistory" :class="{ active: activeTab === 'history' }" @click="activeTab = 'history'" :disabled="!selectedChannelId">
        历史趋势
      </button>
    </div>

    <div class="content-area">
      <div v-show="activeTab === 'overview' && props.showOverview" class="overview-section">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="item in replacementSummaries" :key="item.channelId"
                  ...
                  <td :class="{ 'value-warning': item.currentEfficiency < props.efficiencyThreshold }">
      ...
      <div v-show="activeTab === 'channel' && props.showChannelDetail" class="channel-section">
        <div v-if="panelData" class="channel-detail">
          <div class="detail-header">
            ...
            <span class="status-badge" :class="panelData.status">
              {{ panelData.status === 'normal' ? '正常' : panelData.status === 'warning' ? '告警' : '故障' }}
            </span>
          </div>

          <div class="metrics-grid">
            <div class="metric-card" :class="{ 'warning': panelData.currentEfficiency < props.efficiencyThreshold }">
          ...
          <div class="action-bar">
            ...
            <button v-if="panelData.needsReplacement" class="btn-danger" @click="createWorkOrder(selectedChannelId!)">
              生成更换工单
            </button>
          </div>
        </div>
      ...
      <div v-show="activeTab === 'history' && props.showHistory" class="history-section">
