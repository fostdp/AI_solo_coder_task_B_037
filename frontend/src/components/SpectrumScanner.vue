...
const centerFrequency = ref(3500)
...
const loadChartData = async () => {
  ...
  try {
    chartData.value = await api.getSpectrumChartData(currentStationId.value)
  } catch (e) {
...
const loadScanRecords = async () => {
  if (!currentStationId.value) return
  try {
    scanRecords.value = await api.getSpectrumScanRecords(currentStationId.value, 24)
  } catch (e) {
...
const runSpectrumScan = async () => {
  ...
  scanning.value = true
  try {
    const result = await api.runSpectrumScan({
...
const estimateDoA = async (source: InterferenceSource) => {
  ...
  try {
    doaResult.value = await api.estimateDoA(currentStationId.value, source.id)
  } catch (e) {
...
const applyNullSteering = async () => {
  if (!currentStationId.value) return
  try {
    const result = await api.configureNullSteering({
...
const toggleNullSteering = async (enabled: boolean) => {
  if (!currentStationId.value) return
  try {
    await api.enableNullSteering(currentStationId.value, enabled)
    if (chartData.value) {
...
<template>
  <div class="spectrum-scanner">
    ...
    <div class="tab-switcher">
      <button :class="{ active: activeTab === 'spectrum' }" @click="activeTab = 'spectrum'">
        频谱图
      </button>
      <button :class="{ active: activeTab === 'interference' }" @click="activeTab = 'interference'">
        干扰源列表
      </button>
      <button :class="{ active: activeTab === 'nullsteering' }" @click="activeTab = 'nullsteering'">
        零陷控制
      </button>
    ...
    <div class="content-area">
      <div v-show="activeTab === 'spectrum'" class="spectrum-section">
      ...
      <div v-show="activeTab === 'interference'" class="interference-section">
        <div class="chart-card">
          <h3>检测到的干扰源</h3>
      ...
      <div v-show="activeTab === 'nullsteering'" class="nullsteering-section">
        <div class="chart-card">
          <h3>自适应波束零陷控制</h3>
